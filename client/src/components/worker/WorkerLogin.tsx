...
import { workerLogin } from '../../features/axios/api/worker';
...
const WorkerLogin = () => {
  ...
  const workerIsLoggedIn = useSelector((state:RootState)=>state.workerAuth.WorkerLoggedIn)

  const workerToken = localStorage.getItem("workertoken")

  useEffect(()=>{
    console.log(workerLoginReducer);
  ...
  type === "error"
    ...
    return (
       ...
       <div className="bg-gradient-to-t from-gray-400 to-gray-200  p-8 rounded shadow-md w-full sm:w-96">
        <h2 className="text-2xl font-semibold text-black  mb-6">Worker Login</h2>
        <Formik
       ...
       {({ errors, touched }) => (
        <Form>
          ...
          <div className='flex justify-center mt-3'>
            <a onClick={()=>navigate("/worker/workerRegister")} className="text-sm text-black hover:text-gray-600">
